...
class perturbed(utils.BasePerturbed):
	...
	def save_img(self, m, n, fold_curve='fold', repeat_time=4, fiducial_points = 16, relativeShift_position='relativeShift_v2'):
		...
		perspective_shreshold = random.randint(18, 52)*10 # 280
...
def xgw(args):
	...
	if args.output_path is None:
		save_path = '/lustre/home/gwxie/data/unwarp_new/train/fiducial1024/fiducial1024_v2_a1/'
	else:
		...
		for n in range(args.sys_num, args.sys_num_e):
			...
			for m_n in range(10):
				try:
					...
					# repeat_time = min(max(round(np.random.normal(12, 4)), 1), 20)
					repeat_time = min(max(round(np.random.normal(8, 4)), 1), 16)	# random.randint(1, 2)		# min(max(round(np.random.normal(8, 4)), 1), 12)
					# repeat_time = 0
					process_pool.apply_async(func=saveFold.save_img, args=(m, n, 'fold', repeat_time, fiducial_points, 'relativeShift_v2'))

					# repeat_time = min(max(round(np.random.normal(10, 4)), 1), 18)
...
if __name__ == '__main__':
	...
	parser.add_argument('--sys_num_e', default=11, type=int)
