...
class perturbed(utils.BasePerturbed):
	...
	def save_img(self, m, n, fold_curve='fold', repeat_time=4, fiducial_points = 16, relativeShift_position='relativeShift_v2'):
		...
		reduce_value = np.random.choice([2*2, 4*2, 8*2, 16*2, 24*2, 32*2, 40*2, 48*2], p=[0.02, 0.08, 0.2, 0.3, 0.1, 0.1, 0.1, 0.1])
		...
		reduce_value_v2 = max(np.random.choice(range(0, reduce_value, 2)), 4)
		...
		for repeat_i in range(repeat_time):
			...
			if fold_curve == 'fold':
				fold_curve_random = self.is_perform(1, 0)
				alpha_perturbed = random.randint(70, 130) / 100
			else:
				fold_curve_random = self.is_perform(0, 1)
				alpha_perturbed = random.randint(80, 130) / 100
			...
			''''''
			if fold_curve == 'fold' and self.is_perform(0.5, 0.5):
				...
				if self.is_perform(0.5, 0.5):
					...
					perturbed_v = np.array([random.randint(-6000, 6000) / 100, random.randint(-6000, 6000) / 100])
			''''''
			if fold_curve == 'fold':
		...
		perspective_shreshold = random.randint(24, 40)*10 # 280
...
def xgw(args):
	...
	if args.output_path is None:
		save_path = '/lustre/home/gwxie/data/unwarp_new/train/fiducial1024/fiducial1024_v2_a3/'
	else:
		...
		for n in range(args.sys_num, args.sys_num_e):
			...
			for m_n in range(10):
				try:
					...
					repeat_time = min(max(round(np.random.normal(14, 4)), 1), 24)
					...
					repeat_time = min(max(round(np.random.normal(4, 2)), 1), 10)
...
if __name__ == '__main__':
	...
	parser.add_argument('--sys_num', default=17, type=int)
	parser.add_argument('--sys_num_e', default=22, type=int)
